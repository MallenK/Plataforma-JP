...
| **4 — pulido ✅** | Plantillas de respuesta rápida (`TicketModel::REPLY_TEMPLATES`, desplegable en el form). `reported_urgency` (urgencia del solicitante, informativa; coach/staff la indican y `priority` entra como `media`; **el alumno no la indica** — sin selector, `priority=media`, `reported_urgency=null`). Generador de nº por año atómico (`ticket_counters`, `ON DUPLICATE KEY UPDATE`, reinicia en enero). `/tickets/gestion*` como nombre nuevo con redirección 301 desde `/tickets/admin*`. Migración `2026-09-07-000004`. | 1 columna + 1 tabla | Hecho. |
