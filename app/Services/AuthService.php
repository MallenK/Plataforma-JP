...
class AuthService
{
    ...
    public function attempt(string $email, string $password, bool $remember = false)
    {
        ...
        if (($user['status'] ?? 'active') !== 'active') {
            ...
            return 'Esta cuenta está desactivada. Contacta con el equipo de URPA.';
        }
    ...
    public function createPasswordReset(string $email)
    {
        ...
        $sent = (new MailService())->send(
            $email,
            'Recuperación de contraseña — URPA',
    ...
    private function buildResetEmailHtml(string $name, string $link): string
    {
        return '
            <div style="font-family:sans-serif;max-width:480px;margin:auto">
                <h2>Hola, ' . esc($name) . '</h2>
                <p>Recibimos una solicitud para restablecer tu contraseña en <strong>URPA</strong>.</p>
